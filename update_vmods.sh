...
SCRIPT_URL="https://raw.githubusercontent.com/robertneed20k/gta-v-mods-manager/main/vmods.sh"

# Path to the current script
SCRIPT_PATH="$PREFIX/bin/vmods"
...
update_script() {
    ...
    curl -s -o ~/vmods_latest.sh "$SCRIPT_URL"

    if ! cmp -s ~/vmods_latest.sh "$SCRIPT_PATH"; then
        echo -e "\e[1;32mNew version found. Updating...\e[0m"
        mv ~/vmods_latest.sh "$SCRIPT_PATH"
        chmod +x "$SCRIPT_PATH"
